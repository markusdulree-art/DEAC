...
int main() {
    ...
    {
        // Observing multiple weak families must not satisfy the independent-support rule.
        // Only families whose fused score reaches the monitor threshold count as supporting.
        deac::policy::Engine policy({0.70f, 0.82f, 0.93f, 1, 2, 32});
        for (const char* family : {"process-access", "memory-integrity"}) {
            deac::policy::Evidence e{};
            e.anomaly = 0.40f;
            e.data_quality = 1.0f;
            e.evidence_key = std::string(family) + ":weak";
            e.evidence_family = family;
            policy.add(e);
        }
        const auto r = policy.evaluate();
        assert(r.supporting_families == 0);
        assert(r.decision == deac::policy::Decision::Allow);
    }
