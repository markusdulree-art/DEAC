...
- Maintains a known-good hash set derived only from already-trusted game-root modules; startup observation by itself never turns an unsigned module into trusted content.
...
The current driver/service protocol is **v5** and includes queue-loss accounting, expanded capability state, versioned event payloads, and the canonical evidence/session fields consumed by the service.
...
Evidence-family aggregation
        |
        v
Relationship-aware fusion + independence analysis
        |
        v
Quality/confidence ceiling
        |
        v
Allow / Monitor / Review / Enforce
```

The graph is deliberately an evidence layer rather than a direct ban mechanism. Relationships remain structured provenance rather than being flattened into an anomaly bonus. Family-level fusion applies diminishing returns within a family, cross-family reinforcement across independent families, and telemetry loss imposes a confidence ceiling.
...
- Module assessment records signer certificate thumbprints and a best-effort mapped-image path check; known-good startup hashes contain only already-trusted game-root content, never arbitrary unsigned content.
