...
Runtime::Runtime()
    : settings_(config::Defaults()),
      ...
      graph_(graph::TimingConfig{500, 1000, 2000, 5000}, 4096) {}
...
bool Runtime::start() {
    ...
    policy_config.monitor_threshold = settings_.monitor_threshold;
    policy_config.review_threshold = settings_.review_threshold;
    policy_config.enforce_threshold = settings_.enforce_threshold;
    policy_config.minimum_supporting_events = settings_.minimum_supporting_events;
    policy_config.minimum_supporting_families = settings_.minimum_supporting_families;
    policy_config.max_evidence = 512;
    policy_engine_.configure(policy_config);
    ...
    while (!stopping_.load(std::memory_order_acquire)) {
        ...
        if (now_ms - last_status_poll_ms >= 2000) {
            ...
            if (const auto status = driver.status()) {
                ...
                if (status->queue_dropped > last_dropped) {
                    ...
                    loss.session_id = cs2_identity_.valid() ? cs2_identity_.Key() : std::string{};
                    loss.target = cs2_identity_;
        ...
        if (tracked_pid != 0 && now_ms - last_memory_scan_ms >= 2000) {
            ...
            for (const auto& region : regions) {
                ...
                memory.session_id = target.Key();
                memory.correlation_id = target.Key() + ":region:" + region_key;
                ...
                for (const auto& source : sources) {
                    ...
                    if (corr.handle_before_memory) {
                        ...
                        // Join the same source/target integrity chain used by suspicious
                        // module evidence. Region identity remains in evidence_key, so the
                        // correlation ID describes the explanation chain rather than one
                        // virtual address.
                        memory.correlation_id = target.Key() + ":source:" +
                            source.Key() + ":integrity-chain";
                        // Preserve the memory observation strength. The handle->memory
                        // relationship is retained structurally for policy fusion/audit.
                        memory.anomaly = std::clamp(memory.anomaly, 0.0f, 1.0f);
                        audit::Record rec{};
                        rec.category = "correlation";
                        rec.message = "same-process-instance dangerous handle precedes executable private memory";
                        rec.monotonic_ms = now_ms;
                        rec.session_id = target.Key();
                        rec.target = target;
                        rec.source = source;
                        rec.evidence_key = memory.evidence_key;
                        rec.correlation_id = memory.correlation_id;
                        rec.evidence_family = memory.evidence_family;
                        rec.correlation_edges = memory.correlation_edges;
                        rec.correlation_boost = memory.correlation_boost;
                        rec.anomaly = memory.anomaly;
                        rec.quality = memory.data_quality;
                        audit_.write(rec);
                        break;
                    }
                }
        ...
        if (last_sequence != 0 && event->sequence > last_sequence + 1) {
            ...
            gap.session_id = cs2_identity_.valid() ? cs2_identity_.Key() : std::string{};
            gap.target = cs2_identity_;
        ...
        evidence.session_id = cs2_identity_.valid() ? cs2_identity_.Key() : std::string{};
        ...
        if (type == protocol::EventType::ProtectedHandleAttempt &&
            ...
            evidence.source_birth_token = evidence.source.birth_token;
            evidence.session_id = cs2_identity_.Key();
            if (dangerous) {
                evidence.anomaly = 0.25f;
                evidence.data_quality = 0.35f;
                evidence.evidence_key = "dangerous_handle";
                evidence.evidence_family = "process-access";
                // Stable chain identity: all evidence attributable to the same source/target
                // process-instance relationship shares one correlation ID. The event sequence
                // remains in evidence_key for per-observation provenance.
                evidence.correlation_id = cs2_identity_.Key() + ":source:" +
                    evidence.source.Key() + ":access-chain";
                ...
                evidence.evidence_family = "process-observation";
            }
            ...
            if (module.verdict == modules::Verdict::Suspicious) {
                ...
                for (const auto& source : sources) {
                    ...
                    if (corr.handle_before_module) {
                        ...
                        // Keep the raw module anomaly intact. The graph relationship is
                        // provenance, not a license to manufacture a near-certain scalar.
                        evidence.anomaly = std::clamp(evidence.anomaly, 0.0f, 1.0f);
                        audit::Record rec{};
                        rec.category = "correlation";
                        rec.message = "same-process-instance handle precedes suspicious module state";
                        rec.monotonic_ms = now_ms;
                        rec.sequence = event->sequence;
                        rec.event_type = event->type;
                        rec.session_id = cs2_identity_.Key();
                        rec.target = cs2_identity_;
                        rec.source = source;
                        rec.evidence_key = evidence.evidence_key;
                        rec.correlation_id = evidence.correlation_id;
                        rec.evidence_family = evidence.evidence_family;
                        rec.correlation_edges = evidence.correlation_edges;
                        rec.correlation_boost = evidence.correlation_boost;
                        rec.anomaly = evidence.anomaly;
                        rec.quality = evidence.data_quality;
                        audit_.write(rec);
                        break;
                    }
                }
...
void Runtime::telemetryLoop() {
    while (!stopping_.load(std::memory_order_acquire)) {
        ...
        if (aggregate.valid_count >= 20) {
            ...
            evidence.target = cs2_identity_;
            evidence.session_id = cs2_identity_.Key();
            evidence.correlation_id = cs2_identity_.Key() + ":behavioral-window";
            policy_engine_.add(evidence);
            (void)evidence_.append(evidence);
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(settings_.telemetry_poll_ms));
    ...
    while (!stopping_.load(std::memory_order_acquire)) {
        ...
        if (result.decision != previous) {
            ...
            decision_rec.message = std::string(DecisionName(result.decision)) +
                ...
                " correlated=" + std::to_string(result.correlated_events) +
                " telemetry_integrity=" + std::to_string(result.telemetry_integrity);
            decision_rec.evidence_families = result.supporting_evidence_families;
            decision_rec.evidence_keys = result.supporting_evidence_keys;
            decision_rec.correlation_ids = result.supporting_correlation_ids;
            decision_rec.anomaly = result.confidence;
            decision_rec.quality = result.telemetry_integrity;
