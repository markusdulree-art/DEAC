...
Score Evaluate(const telemetry::Aggregate& a) {
    Score result{};
    if (a.sample_count < 20) return result;
    ...
    // Heuristic-only score. This is evidence, never an automatic ban decision.
    result.anomaly =
    ...
    result.data_quality = std::clamp(static_cast<float>(a.sample_count) / 500.0f, 0.0f, 1.0f);
