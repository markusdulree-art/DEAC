...
const char* ToString(Provenance value) noexcept {
    switch (value) {
        ...
        case Provenance::SignedUntrusted: return "signed-untrusted";
...
ModuleAssessment Inventory::Assess(std::uint64_t pid, const std::filesystem::path& raw_path,
    ...
    bool known_good_hash = false;
    if (!result.sha256.empty()) {
        std::scoped_lock lock(mutex_);
        known_good_hash = known_good_game_hashes_.find(result.sha256) != known_good_game_hashes_.end();
    }

    if (!signed_valid) {
        result.provenance = under_game ? Provenance::GameRootUnsigned : Provenance::ExternalUnsigned;
        // Startup observation is not itself trust. Only a previously verified hash can downgrade
        // an unsigned module to Observe.
        result.verdict = under_game && known_good_hash ? Verdict::Observe : Verdict::Suspicious;
        ...
        if (trusted_signer) {
            // The cryptographic signer identity, not a publisher display string, is the trust
            // anchor. Path is retained as contextual policy input.
            if (under_game) result.provenance = Provenance::ValveSigned;
            else if (under_windows) result.provenance = Provenance::MicrosoftSigned;
            else result.provenance = Provenance::TrustedThirdPartySigned;
            result.verdict = Verdict::Trusted;
            result.anomaly = 0.01f;
        } else {
            result.provenance = Provenance::SignedUntrusted;
            result.verdict = Verdict::Observe;
            // A valid signature whose signer is not in the DEAC trust policy is not malicious
            // by itself. It is observable and can be strengthened by contextual evidence.
            result.anomaly = under_game ? 0.18f : 0.12f;
        }
    }
...
std::vector<ModuleAssessment> Inventory::Refresh(std::uint64_t pid) {
    ...
    std::lock_guard lock(mutex_);
    if (!known_good_established_) {
        for (const auto& item : current) {
            if (!item.sha256.empty() && item.verdict == Verdict::Trusted &&
                IsPathUnder(item.path, game_root_)) {
                known_good_game_hashes_.insert(item.sha256);
            }
        }
        known_good_established_ = true;
    }

    // Re-assess game-root unsigned modules only when their exact hash was previously observed
    // in a cryptographically trusted game-root module. Startup observation alone is never trust.
    for (auto& item : current) {
        if (item.provenance == Provenance::GameRootUnsigned &&
            known_good_game_hashes_.find(item.sha256) != known_good_game_hashes_.end()) {
